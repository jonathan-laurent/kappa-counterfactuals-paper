...
\paragraph{Notation and diction:} We will use the symbol $e$, which in
previous sections referred to an event template, to directly denote an
event. Moreover, when we say that an event \emph{tests} or
\emph{modifies} a site we really mean the tests and actions involved
when matching and rewriting, respectively, the underlying rule in the
mixture $\TSTATE{t}{\tau}$. For example, event $p$ in \RefTrace{}
tests three sites and modifies one. Finally, an event $e$ occuring at
time $t$ is said to be \emph{executable} in trace $\tau$ if the
associated template is realizable in mixture $\TSTATE{\tau}{t}$.
