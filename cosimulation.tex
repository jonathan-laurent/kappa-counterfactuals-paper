...
random contingencies. These contingencies arise from the randomness
that is associated with the ``scheduling'' of event templates, and
that repeatedly determines which realizable event template gets
realized next and when it does. Executing a counterfactual experiment
requires that we separate this randomness from the deterministic
action of rules in order to properly condition on the randomness of
the factual trace. We propose such a decomposition, which is motivated
by a standard construction in physics that justifies the
Doob-Gillespie algorithm used in simulating reaction systems
\cite{gillespie1977exact}.
...
given the opportunity and what it does when it occurs.  It
